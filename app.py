# =========================================
# APP.PY - BOT MONITOR LELANG FINAL FIXED
# =========================================
...
def send_message(lot):
    ...
    penjual = seller.get("namaOrganisasiPenjual", "(tidak diketahui)")
    telepon_penjual = seller.get("telepon", "-")
    alamat_penjual = seller.get("alamat", "-")
    kota_penjual = seller.get("namaKota", "-")
    prov_penjual = seller.get("namaProvinsi", "-")

    # ===================== CARA PENAWARAN =====================
    cara_penawaran = detail.get("content", {}).get("lot", {}).get("caraPenawaran", "-")
    if cara_penawaran:
        cara_penawaran = cara_penawaran.replace("_", " ").title()

    # ===================== UANG JAMINAN =====================
    uang_jaminan = int(detail.get("uangJaminan", 0))

    # ===================== BARANG =====================
    barangs = detail.get("content", {}).get("lot", {}).get("barang", [])
    uraian_list = []
    for b in barangs:
        uraian_list.append(
            f"- {b.get('namaBarang','-')} ({b.get('tahun','-')}, {b.get('warna','-')}, "
...
def main():
    ...
    save_seen(seen)
    if new_count == 0:
        print(f"[INFO] Tidak ada lot baru, semua sudah terkirim")
    else:
